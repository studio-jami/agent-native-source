...
클라이언트 화면을 숨기거나 표시하려면 `@agent-native/core/client`의 `useFeatureFlag(FULL_APP_BUILDING.key)`를 사용하세요. Core는 `get-feature-flags`, `list-feature-flags`, `set-feature-flag`를 제공합니다. 관리는 권한 검사와 감사를 거치며, fleet UI는 엄격히 제한된 A2A 위임으로 같은 앱 로컬 actions를 호출합니다. 평가는 부수 효과가 없습니다. 사용자가 보호된 경험을 실제로 만날 때 서버에서는 `exposeFeatureFlag()`, 마운트된 클라이언트 화면에서는 `useFeatureFlagExposure()`를 사용해 명시적 실험 exposure를 보냅니다. flag 쓰기를 일반 설정 endpoint나 원시 SQL로 처리하지 말고, 인증, 권한 부여, 감사, secret 또는 SSR 캐시를 runtime flag 뒤에 두지 마세요.

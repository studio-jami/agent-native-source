...
その必要はありません。すべてのサンプルアプリは、`agent-native.com` — `mail.agent-native.com`、`calendar.agent-native.com` などでホストされるアプリとしても利用できます。ホストされたバージョンを無料または有料で使用します。ホストされたバージョンが公開していないものを変更したい場合にのみフォークしてください。

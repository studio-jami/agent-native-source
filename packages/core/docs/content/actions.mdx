...
actions through narrowly scoped A2A delegation. Evaluation is side-effect free:
when a user actually sees a gated experience, use `exposeFeatureFlag()` on the
server or `useFeatureFlagExposure()` in the mounted client surface to emit an
explicit experiment exposure. Never route flag writes through generic settings
endpoints or raw SQL, and never put authentication, authorization, audit,
secrets, or SSR cache behavior behind a runtime flag.

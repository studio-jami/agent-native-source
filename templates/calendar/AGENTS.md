...
| `create-event`                       | `--title`, `--start`, `--end`, `--description`, `--location`, `--attendees`, `--eventType default\|outOfOffice\|focusTime\|workingLocation`, `--transparency opaque\|transparent`, `--visibility default\|public\|private`, `--reminderMinutes`, `--remindersUseDefault`, `--addGoogleMeet`, `--addZoom`, `--sendUpdates` | Create event/status block on Google Calendar                           |
| `update-event`                       | `--id`, optional `--title`, `--start`, `--end`, `--recurrence`, `--attendees`, `--transparency opaque\|transparent`, `--visibility default\|public\|private`, `--reminderMinutes`, `--remindersUseDefault`, `--addGoogleMeet`, `--addZoom`, `--sendUpdates`, `--accountEmail`                                             | Update an event or recurrence                                          |
...
Do **not** try to pass `colorId` or color fields to `update-event` for these requests. Google Calendar API limits apply to event mutations, but local app-layer visual changes are supported by `calendar-view-preferences` and `app/lib/event-colors.ts`.

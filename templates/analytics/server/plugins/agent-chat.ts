...
export default createAgentChatPlugin({
  ...
  extraContext: async (event) => {
    ...
    const sourceGuidance =
      ...
      "For broad Gong transcript/call research, use the raw provider/corpus path before relying on convenience actions: call `provider-api-request` with provider=`gong`, POST `/calls/extensive`, body `{ filter: { fromDateTime: ... }, contentSelector: { exposedFields: { parties: true, content: { brief: true, keyPoints: true } } } }`, and `stageAs` or `saveToFile`; for Gong pagination use `pagination: { nextCursorPath: 'records.cursor', cursorBodyPath: 'cursor' }` or `fetchAllPages: { cursorPath: 'records.cursor', cursorBodyPath: 'cursor', itemsPath: 'calls' }`. Then use `run-code` with `providerFetch`, `workspaceRead`, and batched POST `/calls/transcript` calls to join, grep, classify, and aggregate. The `gong-calls` action is a bounded shortcut for quick account checks, not the ceiling for what Gong can answer. " +
